...
public enum GenericDeviceType {
	...
	GENERIC_TYPE_UNKNOWN(Integer.MAX_VALUE);
	
	final int protocolValue;
	
	private GenericDeviceType(int protocolVersion) {
		this.protocolValue = protocolVersion;
	}
	
	public static GenericDeviceType fromProtocolVersion(int protocolVersion) {
		for (GenericDeviceType genericDeviceType : GenericDeviceType.values()) {
			if (genericDeviceType.protocolValue == protocolVersion) {
